...
const FoldText = ({
  ...
  className = '',
  style = {},
  // Extensión: reproduce el pliegue inverso (salida) antes de cambiar el texto.
  phase = 'in',
  onExitComplete
}) => {
  ...
  useEffect(() => {
    ...
    // Fase de salida: se repliega en orden inverso y avisa al terminar.
    if (phase === 'out') {
      killTimeline();
      timelineRef.current = gsap.timeline({
        onComplete: () => exitCallbackRef.current?.()
      });
      timelineRef.current.to(pieces, {
        ...foldedVars,
        duration: activeDuration,
        ease: reduceMotion ? 'power1.in' : 'power3.in',
        stagger: { each: activeStagger, from: 'end' }
      });
    ...
    const play = repeat => {
      ...
      timelineRef.current.fromTo(pieces, foldedVars, unfoldedVars);
